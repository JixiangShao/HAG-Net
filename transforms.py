...
class RandomContrast(object):
    """随机调整图像对比度（仅作用于图像，不改变标签）"""
    def __init__(self, contrast_factor=(0.8, 1.2)):
        """
        Args:
            contrast_factor: 对比度调整因子范围，如(0.8, 1.2)表示随机从0.8到1.2之间取值
        """
...
class RandomGamma(object):
    ...
    def __init__(self, gamma_limit=(0.7, 1.5)):
        """
        Args:
            gamma_limit: 伽马值范围，如(0.7, 1.5)表示随机从0.7到1.5之间取值
        """
        self.gamma_limit = gamma_limit

    def __call__(self, image, target):
        if random.random() < 0.5:  # 50%概率应用伽马校正
