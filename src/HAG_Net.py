...
from module.AttentionGate import AttentionGate
...
class Up(nn.Module):
    def __init__(self, up_in_channels, skip_in_channels, out_channels, bilinear=True, use_graph=False, use_hlaem=True):
        ...
        self.att_gate = AttentionGate(
            encoder_channels=skip_in_channels,  # Channel number of encoder skip feature
            decoder_channels=decoder_channels  # Channel number of feature after decoder upsampling
        )
        # Convolution fusion layer (input channels = encoder feature channels + decoder feature channels)
        self.conv = DoubleConv(skip_in_channels + decoder_channels, out_channels)

        # Apply HLAEM in key decoding stages
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, graph_feat=None) -> torch.Tensor:
        ...
        # Feature fusion via attention gate
        x = self.att_gate(encoder_feat=x2, decoder_feat=x1)  # x2 denotes encoder feature, x1 denotes decoder feature

        x = self.conv(x)
        x = self.hlaem_fusion(x)  # HLAEM fusion at critical position

        # Apply GNN attention if enabled
        ...
        x = x * spatial_att + x  # Residual connection
        return x
...
class VesselGraphBranch(nn.Module):
    """Vessel Graph Neural Network Branch"""
    ...
    def forward(self, x, edge_index, edge_attr):
        # Calculate node feature similarity as dynamic edge weight
        src, dst = edge_index
        ...
        # Second layer graph convolution with residual connection
        residual = x
...
class HAG_Net(nn.Module):
    """UNet_HLAEM Network Integrated with GNN"""
    ...
    def __init__(self,
        ...
        # Channel numbers for encoder layers
        self.c1 = base_c
        self.c2 = base_c * 2
        self.c3 = base_c * 4
        self.c4 = base_c * 8
        ...
        # Add HLAEM at skip connection fusion nodes (Key Position 4)
        ...
        if self.use_graph:
            self.graph_branch = VesselGraphBranch(
                in_channels=self.c3 + 64,  # c3 feature map + positional encoding
                hidden_channels=64,
                edge_dim=self.c3
            )
    ...
    def extract_vessel_centers(self, enc_feat):
        ...
        for b in range(batch_size):
            # Calculate feature intensity norm
    ...
    def build_knn_graph(self, centers,features= None):
        ...
        # Dynamically adjust neighbor count
        k_neighbors = min(self.k_neighbors + 2, centers_np.shape[0] - 1)  # Increase neighbor quantity

        # Construct graph based on spatial Euclidean distance
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Encoder feature extraction pipeline
        x1 = self.in_conv(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)

        # HLAEM enhancement at bottleneck layer
        x4 = self.hlaem_bottleneck(x4)

        # HLAEM enhancement at skip connection layers
        x3 = self.hlaem_skip3(x3)
        x1 = self.hlaem_skip1(x1)

        # GNN feature processing pipeline
        graph_feat_map = None
        if self.use_graph:
            ...
            for b in range(batch_size):
                ...
                # Node feature composition: encoder feature + positional encoding
                ...
                # Scatter graph node features back to spatial feature map
        ...
        x = self.up1(x4, x3)  # High-level decoding with HLAEM
        x = self.up2(x, x2)  # Mid-level decoding without HLAEM
        x = self.up3(x, x1, graph_feat=graph_feat_map)  # Final decoding with HLAEM
...
if __name__ == "__main__":
    # Test input and output tensor dimensions of the network
    model = HAG_Net(in_channels=3, num_classes=1, use_graph=True)
    input_tensor = torch.randn(1, 3, 256, 256)
    output = model(input_tensor)
    print(f"Input shape: {input_tensor.shape}")
    print(f"Output shape: {output['out'].shape}")  # Output tensor shape should be [1, 1, 256, 256]