...
class VGG16UNet(nn.Module):
    def __init__(self, num_classes, pretrain_backbone: bool = False):
        ...
        #backbone = efficientnet_b0(pretrained=pretrain_backbone)

        if pretrain_backbone:
            # 适配torchvision新版本的权重参数（替换原来的pretrained）
            backbone = vgg16_bn(weights=VGG16_BN_Weights.IMAGENET1K_V1)
        else:
            backbone = vgg16_bn(weights=None)  # 不使用预训练权重

        backbone = backbone.features  # VGG16的特征提取部分
