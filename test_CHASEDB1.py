...
# -------------------------- Configuration Parameters (Modify log storage path) --------------------------
data_root = "./CHASEDB1"
weights_path = "./save_weights_CHASEDB1/HAG_Net_model.pth"
num_classes = 1  # Binary segmentation (background + foreground vessel)
mean = (0.628, 0.224, 0.037)
std = (0.221, 0.110, 0.036)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
save_pred = True
save_dir = "results_CHASEDB1/test_pred_HAG_Net"  # Directory for predicted segmentation images (adjust here if needed)

log_file_path = "results_CHASEDB1/test_pred_HAG_Net/test_results_HAG_Net.txt"
...
if __name__ == "__main__":
    ...
    test_gt_dir = os.path.join(data_root, "test", "manual")

    # 5. Scan and validate test image samples
    img_names = [f for f in os.listdir(test_img_dir) if f.endswith(".jpg")]
    if not img_names:
    ...
    for img_idx, img_name in enumerate(img_names, 1):
        ...
        roi_name = img_name.replace(".jpg", "_test_mask.gif")
        ...
        gt_name = img_name.replace(".jpg", "_1stHO.png")
    ...
    if len(img_names) > 0:
        ...
        avg_f1 = total_f1 / len(img_names)  # New average F1 metric
        avg_auc = total_auc / len(img_names)  # New average AUC metric
