...
class SegmentationPresetTrain:
    def __init__(self, base_size, crop_size, hflip_prob=0.5, vflip_prob=0.5,
                 mean = (0.628, 0.224, 0.037), std = (0.221, 0.110, 0.036)):
        # Calculate minimum and maximum random resize sizes based on the base input size
        ...
        # Build data augmentation transform list, starting with random resizing
        trans = [T.RandomResize(min_size, max_size)]
        # Add random horizontal flip if flip probability is greater than 0
        if hflip_prob > 0:
            trans.append(T.RandomHorizontalFlip(hflip_prob))
        # Add random vertical flip if flip probability is greater than 0
        if vflip_prob > 0:
            trans.append(T.RandomVerticalFlip(vflip_prob))
        trans.append(T.RandomContrast(contrast_factor=(0.8, 1.2)))
        trans.append(T.RandomGamma(gamma_limit=(0.7, 1.5)))

        # Append random cropping, tensor conversion and normalization operations to transform list
        trans.extend([
            T.RandomCrop(crop_size),  # Randomly crop image to specified size
            T.ToTensor(),  # Convert image from PIL format to tensor format
            T.Normalize(mean=mean, std=std),  # Perform normalization on image
        ])
        # Combine all data augmentation operations into one transform pipeline
        self.transforms = T.Compose(trans)

    def __call__(self, img, target):
        # Apply all transforms to input image and label when called
        return self.transforms(img, target)


class SegmentationPresetEval:
    def __init__(self, mean = (0.628, 0.224, 0.037), std = (0.221, 0.110, 0.036)):
        # Define transform pipeline for evaluation (validation/test) mode
    ...
    def __call__(self, img, target):
        # Apply evaluation transforms to input image and label when called
        return self.transforms(img, target)


def get_transform(train, mean = (0.628, 0.224, 0.037), std = (0.221, 0.110, 0.036)):
    base_size = 960  # Define base image resolution
    crop_size = 720  # Define target cropped image resolution

    if train:
        # Return training augmentation config if in training mode
        return SegmentationPresetTrain(base_size, crop_size, mean=mean, std=std)
    else:
        # Return evaluation transform config if in validation/test mode
        return SegmentationPresetEval(mean=mean, std=std)


def create_model(num_classes):
    # Initialize UNet model, input channels set to 3 for RGB images, output classes equal to num_classes
...
def main(args):
    ...
    # File to save training and validation metrics
    results_file = "HAG_Net_CHASEDB1 {}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    # Create training and validation dataset instances
    train_dataset = CHASEDB1Dataset(args.data_path,
                                 train=True,
                                 transforms=get_transform(train=True, mean=mean, std=std))

    val_dataset = CHASEDB1Dataset(args.data_path,
                               train=False,
                               transforms=get_transform(train=False, mean=mean, std=std))

    num_workers = 0 if os.name == 'nt' else min([os.cpu_count() // 2, 8])  # Calculate available dataloader workers, cap maximum worker count
    ...
    # Use safe evaluation wrapper in training script
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            # Update best dice score if current dice exceeds historical maximum
            if best_dice < dice:
                best_dice = dice
            else:
                continue
        # Package all training states for checkpoint saving
        ...
        if args.save_best is True:
            torch.save(save_file, "save_weights_CHASEDB1/HAG_Net_model.pth")
        else:
            torch.save(save_file, "save_weights_CHASEDC1/HAG_Net_model_{}.pth".format(epoch))
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="./", help="CHASEDB1 dataset root directory")
...
if __name__ == '__main__':
    ...
    dataset = CHASEDB1Dataset(root=".", train=True, transforms=transform)
    img, mask = dataset[0]  # img shape: (3, 640, 640), mask shape: (1, 640, 640)
    mean = (0.628, 0.224, 0.037)
    std = (0.221, 0.110, 0.036)
    ...
    if not os.path.exists("./save_weights_CHASEDB1"):
        os.mkdir("./save_weights_CHASEDB1")
