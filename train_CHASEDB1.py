...
from src import UNet, ResNetUNet, VGG16UNet, MobileV3Unet, MMUNet, R2U_Net, SegNet, NestedUNet, get_fcn_model, \
    create_ce_net, UNet_HLAEM, UNet_HLAEM_GNN,HAG_Net
...
def get_transform(train, mean = (0.628, 0.224, 0.037), std = (0.221, 0.110, 0.036)):
    base_size = 960  # 定义基础图像尺寸
    crop_size = 720  # 定义裁剪后的图像尺寸
...
def main(args):
    ...
    num_workers = 0 if os.name == 'nt' else min([os.cpu_count() // 2, 8])  # 计算可用的 worker 数量，限制在最小的工作进程数和一些条件下的最小值
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            ...
            torch.save(save_file, "save_weights_CHASEDC1/UNet_HLAEM_GNN_best_model_{}.pth".format(epoch))
    # 计算总训练时间并打印
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="./", help="CHASEDB1 root")
...
if __name__ == '__main__':
    ...
    img, mask = dataset[0]  # img: (3, 640, 640), mask: (1, 640, 640)
    mean = (0.628, 0.224, 0.037)
    std = (0.221, 0.110, 0.036)
    # 反归一化图像（用于可视化）
    ...
    plt.subplot(122)
    plt.imshow(mask.squeeze(), cmap="gray")  # 掩码是单通道，需挤压维度
