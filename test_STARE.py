...
from my_dataset import CHASEDB1Dataset
import datetime
...
data_root = "./STARE"
weights_path = "./save_weights_STARE/HAG_Net_model.pth"
...
save_pred = True
save_dir = "results_STARE/test_pred_HAG_Net"  # Directory to store predicted segmentation images (modify path here if needed)

log_file_path = "results_STARE/test_pred_HAG_Net/test_results_HAG_Net.txt"
...
def calculate_metrics(pred_roi, gt_roi, pred_prob_roi):
    ...
    # Basic segmentation evaluation metrics
    acc = (tp + tn) / (tp + tn + fp + fn + 1e-6) if (tp + tn + fp + fn) != 0 else 0.0
    se = tp / (tp + fn + 1e-6) if (tp + fn) != 0 else 0.0  # Sensitivity (Foreground Recall)
    ...
    precision = tp / (tp + fp + 1e-6) if (tp + fp) != 0 else 0.0  # Precision rate
    ...
    # Handle extreme case: AUC is meaningless if all labels belong to single class, return 0.5
    if len(np.unique(gt_roi)) < 2:
        auc = 0.5
    else:
        try:
            auc = roc_auc_score(gt_roi, pred_prob_roi)  # Compute AUC using foreground probability values
        except:
            auc = 0.5  # Default fallback value when calculation fails
...
if __name__ == "__main__":
    ...
    log_header = f"==================== Test Started ====================\n" \
                 ...
                 f"Model Weight File Path: {weights_path}\n" \
                 f"Test Dataset Root Directory: {data_root}\n" \
                 f"Prediction Output Save Directory: {save_dir}\n" \
    ...
    write_log(f"Model loaded successfully! Input channels: 3, Output category count: {num_classes + 1}", log_file_path)

    # 4. Image preprocessing pipeline and test dataset folder paths
    ...
    test_gt_dir = os.path.join(data_root, "test", "manual2")

    # 5. Scan and validate test image samples
    img_names = [f for f in os.listdir(test_img_dir) if f.endswith(".ppm")]
    if not img_names:
    ...
    for img_idx, img_name in enumerate(img_names, 1):
        # 6.1 Load data of single test sample
        img_path = os.path.join(test_img_dir, img_name)
        img = Image.open(img_path).convert('RGB')

        roi_name = img_name.replace(".ppm", "_mask.gif")
        roi_path = os.path.join(test_roi_dir, roi_name)
        roi_mask = Image.open(roi_path).convert('L')
        roi = np.array(roi_mask) == 255  # Valid ROI region (True = pixels to evaluate)

        gt_name = img_name.replace(".ppm", ".ah.ppm")
        ...
        # 6.2 Run inference on single sample
        img_tensor = data_transform(img).unsqueeze(0).to(device)
        with torch.no_grad():
            outputs = model(img_tensor)
            logits = outputs['out']  # Raw model output before softmax activation
            pred_prob = torch.softmax(logits, dim=1)[:, 1, ...].squeeze(0).cpu().numpy()  # Foreground probability map for AUC calculation
            pred = logits.argmax(1).squeeze(0).cpu().numpy()  # Binary segmentation prediction (0/1) for other metrics

        # 6.3 Calculate and accumulate metrics for current sample
        acc, se, sp, dice, mIoU, f1, auc = calculate_metrics(
            pred_roi=pred[roi],  # Binary prediction cropped to valid ROI
            gt_roi=gt[roi],  # Ground truth label cropped to valid ROI
            pred_prob_roi=pred_prob[roi]  # Foreground probability cropped to valid ROI for AUC
        )
        # Accumulate values of all metrics
        total_acc += acc
        total_se += se
        total_sp += sp
        total_dice += dice
        total_mIoU += mIoU
        total_f1 += f1  # New metric accumulator
        total_auc += auc  # New metric accumulator

        # 6.4 Write log record for single sample with all metrics
        ...
        # 6.5 Save predicted segmentation mask of current sample
        if save_pred:
            pred_mask = np.zeros_like(roi, dtype=np.uint8)
            pred_mask[roi] = pred[roi] * 255  # Convert prediction values to 0/255 within ROI area
            save_path = os.path.join(save_dir, img_name.replace(".ppm", "_pred.png"))
            Image.fromarray(pred_mask).save(save_path)
            write_log(f"  Prediction mask saved to: {save_path}\n", log_file_path)

    # ==================================================
    # Calculate and print average metrics for full test set
    # ==================================================
    write_log("\n" + "=" * 50, log_file_path)
    if len(img_names) > 0:
        # Compute average value across all test samples
        ...
        avg_f1 = total_f1 / len(img_names)  # Average F1 score
        avg_auc = total_auc / len(img_names)  # Average AUC score

        # Log overall test set performance results
        global_log = "Overall Test Set Performance\n" \
                     ...
                     f"Average SE:   {avg_se:.4f}  (Sensitivity / Foreground Recall Rate)\n" \
                     f"Average SP:   {avg_sp:.4f}  (Specificity / Background Precision Rate)\n" \
                     f"Average Dice: {avg_dice:.4f}  (Overlap coefficient of foreground class)\n" \
                     f"Average mIoU: {avg_mIoU:.4f}  (Mean IoU of background and foreground categories)\n" \
                     f"Average F1:   {avg_f1:.4f}  (Harmonic mean of precision and recall)\n" \
                     f"Average AUC:  {avg_auc:.4f}  (Area under ROC curve, representing ranking discriminability)\n" \
                     "=================================="
        write_log(global_log, log_file_path)
    else:
        write_log("No test samples have been processed!", log_file_path)

    # 7. Write test completion footer log
    end_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    end_log = f"\n==================== Test Finished ====================\n" \
              f"Completion Timestamp: {end_time}\n" \
              f"Full test log saved at path: {log_file_path}\n" \
