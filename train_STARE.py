...
from src import UNet, ResNetUNet, VGG16UNet, MobileV3Unet, MMUNet, R2U_Net, SegNet, NestedUNet, get_fcn_model, \
    create_ce_net, UNet_HLAEM, UNet_HLAEM_GNN, HAG_Net,HAG2Net
...
def get_transform(train, mean = (0.5889, 0.3338, 0.1134), std = (0.3530, 0.1921, 0.1091)):
    base_size = 700  # 定义基础图像尺寸
    crop_size = 640   # 定义裁剪后的图像尺寸
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            torch.save(save_file, "save_weights_STARE/HAG2Net_model.pth")
        else:
            torch.save(save_file, "save_weights_STARE/UNet_HLAEM_GNN_best_model_{}.pth".format(epoch))
    # 计算总训练时间并打印
...
def parse_args():
    ...
    parser.add_argument("--data-path", default="./", help="STARE root")
